...
```python
from orchestration.cli.version_guard import validate_version_change
...
**MANDATORY PATTERN**: `^[a-z][a-z0-9_]*$`

ALL component names MUST:
- Start with lowercase letter
- Contain ONLY: lowercase letters, numbers, underscores
- NO hyphens, spaces, uppercase, or special characters

**Examples**:
- ✅ `auth_service`, `payment_api`, `user_lib`, `audio_processor`
- ❌ `auth-service` (hyphens break Python imports)
- ❌ `AuthService` (uppercase not allowed)
- ❌ `payment api` (spaces not allowed)

**Validation**:
ALL component creation MUST call validator first:
```python
from orchestration.verification.system.component_name_validator import validate_component_name

is_valid, message = validate_component_name(name)
if not is_valid:
    raise InvalidComponentNameError(message)
```

**Why This Matters**:
- Python: `from components.auth-service` is syntax error
- Hyphens caused 20% project failure rate in v0.2.0
- Universal convention works across ALL languages
...
Read orchestration configuration from `orchestration/config/orchestration.json`:
...
**PROJECT VERSION**: {{PROJECT_VERSION}} (Check orchestration/config/project_metadata.json)
...
```bash
cat orchestration/config/project_metadata.json | jq '.lifecycle_state, .breaking_changes_policy'
```
...
3. **Atomic Commit**: All components commit together
   ...
   python orchestration/atomic_commit.py \
     --components user_service,auth_service,api_gateway \
...
| `{{PROJECT_VERSION}}` | From `orchestration/config/project_metadata.json` | `0.1.0` |
...
#   {{COMPONENT_NAME}} → payment_api
#   {{PROJECT_VERSION}} → 0.1.0 (from orchestration/config/project_metadata.json)
...
```bash
# Good (absolute)
/workspaces/my-project/components/auth_service
...
max_agents=$(cat orchestration/config/orchestration.json | grep max_parallel_agents | awk '{print $2}' | tr -d ',')
# Default: 3
```
...
#   - payment_api (position 1)
#   - notification_service (position 2)

# [Wait for Task 1 to complete]
# [Task 1 complete - backend_api finished]
...
from orchestration.validation.completion_verifier import CompletionVerifier
...
3. **Create ADR**: Document splitting decision
   ```bash
   python orchestration/adr_generator.py create \
     --title "Split user_api into user_service and auth_service" \
     --status proposed
   ```
...
- ADR-014: Split user_api into user_service and auth_service (Accepted)
