//! Windows WebView implementation using WebView2
//!
//! This module provides WebView2-specific WebView integration for Windows systems.
//! WebView2 is Microsoft's modern web rendering engine based on Chromium/Edge.

use crate::errors::{Error, Result};
use crate::platform::WebViewConfig;
use std::path::PathBuf;
use std::sync::{Arc, Mutex};

/// Windows WebView wrapper using WebView2
///
/// This struct wraps the wry WebView and provides Windows-specific functionality
/// including WebView2 configuration, COM initialization, and Edge-based rendering.
#[cfg(feature = "gui")]
pub struct WindowsWebView {
    /// The underlying wry WebView
    webview: wry::WebView,

    /// Configuration used to create this WebView
    config: WebViewConfig,

    /// Current URL (for tracking navigation)
    current_url: Arc<Mutex<Option<String>>>,

    /// Load state tracking
    is_loading: Arc<Mutex<bool>>,
}

/// Headless mode stub for testing
#[cfg(not(feature = "gui"))]
pub struct WindowsWebView {
    #[allow(dead_code)] // Used in GUI mode and tests
    config: WebViewConfig,
    current_url: Arc<Mutex<Option<String>>>,
    is_loading: Arc<Mutex<bool>>,
}

impl WindowsWebView {
    /// Create a new Windows WebView with the given configuration
    ///
    /// # Arguments
    ///
    /// * `config` - WebView configuration including WebView2-specific settings
    ///
    /// # Returns
    ///
    /// A Result containing the WindowsWebView or an error if initialization fails
    ///
    /// # Errors
    ///
    /// Returns an error if:
    /// - Configuration validation fails
    /// - COM initialization fails
    /// - WebView2 runtime is not available
    /// - WebView2 environment creation fails
    ///
    /// # Example
    ///
    /// ```no_run
    /// use webview_integration::platform::{WebViewConfig, WindowsWebView};
    ///
    /// let config = WebViewConfig::new()
    ///     .with_javascript(true)
    ///     .with_devtools(true);
    ///
    /// let webview = WindowsWebView::new(config).unwrap();
    /// ```
    #[cfg(feature = "gui")]
    pub fn new(config: WebViewConfig) -> Result<Self> {
        // Validate configuration
        config.validate()?;

        // Initialize COM (WebView2 requires COM)
        Self::initialize_com()?;

        // Setup WebView2 environment
        Self::setup_environment(config.cache_dir.clone())?;

        // Build wry WebView with configuration
        use tao::{dpi::LogicalSize, event_loop::EventLoop, window::WindowBuilder};
        use wry::WebViewBuilder;

        // Create event loop (in production, browser_shell manages this)
        let event_loop = EventLoop::new();

        // Create window
        let window = WindowBuilder::new()
            .with_title("FrankenBrowser")
            .with_inner_size(LogicalSize::new(1024, 768))
            .build(&event_loop)
            .map_err(|e| Error::Initialization(format!("Failed to create window: {}", e)))?;

        // Build WebView with configuration
        let mut builder = WebViewBuilder::new();

        // Set initial URL
        builder = builder.with_url("about:blank");

        // Configure user agent
        if let Some(user_agent) = &config.user_agent {
            builder = builder.with_user_agent(user_agent);
        }

        // Add initialization script
        if let Some(init_script) = &config.init_script {
            builder = builder.with_initialization_script(init_script);
        }

        // Enable/disable devtools
        #[cfg(debug_assertions)]
        {
            builder = builder.with_devtools(config.enable_devtools);
        }

        // Build the WebView
        let webview = builder
            .build(&window)
            .map_err(|e| Error::Initialization(format!("Failed to create WebView2: {}", e)))?;

        // Create the WindowsWebView instance
        let instance = Self {
            webview,
            config,
            current_url: Arc::new(Mutex::new(Some("about:blank".to_string()))),
            is_loading: Arc::new(Mutex::new(false)),
        };

        Ok(instance)
    }

    /// Headless mode constructor (for testing on non-Windows platforms)
    #[cfg(not(feature = "gui"))]
    pub fn new(config: WebViewConfig) -> Result<Self> {
        config.validate()?;

        Ok(Self {
            config,
            current_url: Arc::new(Mutex::new(None)),
            is_loading: Arc::new(Mutex::new(false)),
        })
    }

    /// Initialize COM (Component Object Model)
    ///
    /// WebView2 requires COM to be initialized. This is typically done
    /// automatically by wry, but we provide an explicit initialization
    /// method for clarity.
    ///
    /// # Returns
    ///
    /// Ok(()) if COM initialization succeeds
    ///
    /// # Note
    ///
    /// On Windows, wry handles COM initialization internally. This method
    /// is a placeholder for any additional Windows-specific setup.
    #[allow(dead_code)] // Used in GUI mode
    fn initialize_com() -> Result<()> {
        // COM initialization is handled by wry/WebView2
        // CoInitializeEx(NULL, COINIT_APARTMENTTHREADED) is called internally
        // This method is a placeholder for explicit COM setup if needed
        Ok(())
    }

    /// Setup WebView2 environment
    ///
    /// Configures the WebView2 runtime environment, including user data folder
    /// for cookies, cache, and other persistent data.
    ///
    /// # Arguments
    ///
    /// * `user_data_folder` - Optional path for WebView2 user data (cache, cookies, etc.)
    ///
    /// # Returns
    ///
    /// Ok(()) if environment setup succeeds
    ///
    /// # Note
    ///
    /// WebView2 user data folder typically goes in:
    /// - `%LOCALAPPDATA%\FrankenBrowser\WebView2` (default)
    /// - Custom path if specified in config
    #[allow(dead_code)] // Used in GUI mode
    fn setup_environment(user_data_folder: Option<PathBuf>) -> Result<()> {
        // Ensure user data folder exists
        if let Some(data_folder) = user_data_folder {
            std::fs::create_dir_all(&data_folder).map_err(|e| {
                Error::Initialization(format!("Failed to create user data folder: {}", e))
            })?;
        }

        // WebView2 environment creation is handled by wry
        // The CoreWebView2Environment is created with the specified user data folder
        // This method ensures the directory exists for wry to use
        Ok(())
    }

    /// Configure user agent string
    ///
    /// Sets a custom user agent for all HTTP requests.
    ///
    /// # Arguments
    ///
    /// * `agent` - The user agent string to use
    ///
    /// # Returns
    ///
    /// Ok(()) if configuration succeeds
    ///
    /// # Note
    ///
    /// User agent should be set during WebView creation. Changing it
    /// on an existing WebView requires recreation.
    #[allow(dead_code)] // Part of platform API, tested
    fn configure_user_agent(&mut self, agent: &str) -> Result<()> {
        if agent.is_empty() {
            return Err(Error::Initialization(
                "User agent cannot be empty".to_string(),
            ));
        }

        // Update config
        self.config.user_agent = Some(agent.to_string());

        // For wry/WebView2, user agent is set during WebView creation
        // Changing it on an existing WebView would require recreation
        Ok(())
    }

    /// Setup navigation event handlers
    ///
    /// Installs handlers for WebView2 navigation events:
    /// - NavigationStarting: When navigation begins
    /// - NavigationCompleted: When navigation finishes
    /// - SourceChanged: When URL changes
    ///
    /// # Returns
    ///
    /// Ok(()) if handlers are installed successfully
    #[allow(dead_code)] // Part of platform API, tested
    fn setup_navigation_handlers(&self) -> Result<()> {
        // Navigation callbacks are configured through wry's WebViewBuilder
        // This is a placeholder for the interface
        Ok(())
    }

    /// Inject initialization script
    ///
    /// Adds a script that runs on every page load (before page scripts).
    ///
    /// # Arguments
    ///
    /// * `script` - JavaScript code to inject
    ///
    /// # Returns
    ///
    /// Ok(()) if injection succeeds
    #[allow(dead_code)] // Part of platform API, tested
    fn inject_init_script(&mut self, script: &str) -> Result<()> {
        if script.is_empty() {
            return Err(Error::ScriptExecution("Script cannot be empty".to_string()));
        }

        // Update config with new init script
        self.config.init_script = Some(script.to_string());

        // For wry, init scripts are set during WebView creation
        // To add a new init script to an existing WebView would require recreation
        // or using evaluate_script on each navigation
        Ok(())
    }

    /// Navigate to a URL
    ///
    /// Loads the specified URL in the WebView. Fires navigation events:
    /// - NavigationStarting: When navigation begins
    /// - NavigationCompleted: When page fully loads
    ///
    /// # Arguments
    ///
    /// * `url` - The URL to navigate to (must be valid URL string)
    ///
    /// # Returns
    ///
    /// Ok(()) if navigation starts successfully
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use webview_integration::platform::{WebViewConfig, WindowsWebView};
    /// # let config = WebViewConfig::new();
    /// # let mut webview = WindowsWebView::new(config).unwrap();
    /// webview.navigate("https://example.com").unwrap();
    /// ```
    pub fn navigate(&mut self, url: &str) -> Result<()> {
        // Validate URL
        if url.is_empty() {
            return Err(Error::Navigation("URL cannot be empty".to_string()));
        }

        // Parse URL to validate format
        let _parsed_url =
            url::Url::parse(url).map_err(|e| Error::Navigation(format!("Invalid URL: {}", e)))?;

        // Set loading state
        {
            let mut loading = self.is_loading.lock().unwrap();
            *loading = true;
        }

        #[cfg(feature = "gui")]
        {
            // Navigate using wry
            self.webview
                .load_url(_parsed_url.as_str())
                .map_err(|e| Error::Navigation(format!("Failed to navigate: {}", e)))?;
        }

        // Update current URL
        {
            let mut current = self.current_url.lock().unwrap();
            *current = Some(url.to_string());
        }

        Ok(())
    }

    /// Execute JavaScript in the WebView
    ///
    /// Executes the provided JavaScript code in the WebView2 context.
    /// Note: wry's evaluate_script doesn't return script results directly;
    /// use IPC handlers for bidirectional communication.
    ///
    /// # Arguments
    ///
    /// * `script` - The JavaScript code to execute
    ///
    /// # Returns
    ///
    /// Ok with "executed" string on success
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use webview_integration::platform::{WebViewConfig, WindowsWebView};
    /// # let config = WebViewConfig::new();
    /// # let mut webview = WindowsWebView::new(config).unwrap();
    /// webview.execute_script("console.log('Hello from Rust!')").unwrap();
    /// ```
    pub fn execute_script(&self, script: &str) -> Result<String> {
        if script.is_empty() {
            return Err(Error::ScriptExecution("Script cannot be empty".to_string()));
        }

        #[cfg(feature = "gui")]
        {
            self.webview
                .evaluate_script(script)
                .map_err(|e| Error::ScriptExecution(format!("Script execution failed: {}", e)))?;
            Ok("executed".to_string())
        }

        #[cfg(not(feature = "gui"))]
        {
            // In headless mode, simulate execution
            Ok("null".to_string())
        }
    }

    /// Get the current URL
    ///
    /// Returns the URL currently loaded in the WebView, or None if no page is loaded.
    pub fn current_url(&self) -> Option<String> {
        let current = self.current_url.lock().unwrap();
        current.clone()
    }

    /// Check if the WebView is currently loading
    ///
    /// Returns true if a navigation is in progress.
    pub fn is_loading(&self) -> bool {
        let loading = self.is_loading.lock().unwrap();
        *loading
    }

    /// Get the WebView2 runtime version
    ///
    /// Returns the version string of the installed WebView2 runtime.
    ///
    /// # Returns
    ///
    /// Version string (e.g., "109.0.1518.78")
    ///
    /// # Note
    ///
    /// The actual version detection would require querying the WebView2 runtime.
    /// This implementation returns a placeholder.
    pub fn get_webview2_version() -> String {
        #[cfg(feature = "gui")]
        {
            // In a full implementation, this would call:
            // CoreWebView2Environment::GetAvailableBrowserVersionString()
            // For now, return a placeholder
            "WebView2 Runtime (Chromium-based)".to_string()
        }

        #[cfg(not(feature = "gui"))]
        {
            "WebView2 (headless mode)".to_string()
        }
    }
}

/// Get WebView2 version (module-level function)
pub fn get_webview2_version() -> String {
    WindowsWebView::get_webview2_version()
}

#[cfg(test)]
mod tests {
    use super::*;

    // ========================================
    // RED PHASE: Tests for WindowsWebView::new()
    // ========================================

    #[test]
    #[cfg(target_os = "windows")]
    fn test_windows_webview_new_with_default_config() {
        let config = WebViewConfig::default();
        let result = WindowsWebView::new(config);
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_windows_webview_new_with_custom_config() {
        let config = WebViewConfig::new()
            .with_javascript(true)
            .with_devtools(true)
            .with_user_agent("FrankenBrowser/1.0 (Windows)".to_string());

        let result = WindowsWebView::new(config);
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_windows_webview_new_with_cache_dir() {
        let temp_dir = std::env::temp_dir().join("frankenbrowser_test_windows");
        let config = WebViewConfig::new().with_cache_dir(temp_dir.clone());

        let result = WindowsWebView::new(config);
        assert!(result.is_ok());

        // Cleanup
        let _ = std::fs::remove_dir_all(&temp_dir);
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_windows_webview_new_with_cookies() {
        let temp_dir = std::env::temp_dir().join("frankenbrowser_test_cookies_windows");
        let cookies_path = temp_dir.join("cookies.db");
        let config = WebViewConfig::new().with_cookies_path(cookies_path);

        let result = WindowsWebView::new(config);
        assert!(result.is_ok());

        // Cleanup
        let _ = std::fs::remove_dir_all(&temp_dir);
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_windows_webview_new_with_init_script() {
        let init_script = "console.log('WebView2 initialized');".to_string();
        let config = WebViewConfig::new().with_init_script(init_script);

        let result = WindowsWebView::new(config);
        assert!(result.is_ok());
    }

    // ========================================
    // Cross-platform tests (run on all platforms)
    // ========================================

    #[test]
    fn test_windows_webview_headless_creation() {
        // This test works on all platforms in headless mode
        let config = WebViewConfig::default();
        #[cfg(not(feature = "gui"))]
        {
            let result = WindowsWebView::new(config);
            assert!(result.is_ok());
        }
        #[cfg(feature = "gui")]
        {
            // Skip on non-Windows platforms in GUI mode
            #[cfg(not(target_os = "windows"))]
            {
                // Test would fail on non-Windows, so skip
                assert!(true);
            }
            #[cfg(target_os = "windows")]
            {
                let result = WindowsWebView::new(config);
                assert!(result.is_ok());
            }
        }
    }

    // ========================================
    // RED PHASE: Tests for navigate()
    // ========================================

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_valid_url() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.navigate("https://example.com");
        assert!(result.is_ok());
        assert_eq!(
            webview.current_url(),
            Some("https://example.com".to_string())
        );
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_empty_url_fails() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.navigate("");
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), Error::Navigation(_)));
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_invalid_url_fails() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.navigate("not a url");
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), Error::Navigation(_)));
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_updates_loading_state() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        assert!(!webview.is_loading());

        let result = webview.navigate("https://example.com");
        assert!(result.is_ok());
        assert!(webview.is_loading());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_file_url() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.navigate("file:///C:/test.html");
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_navigate_data_url() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.navigate("data:text/html,<h1>Hello</h1>");
        assert!(result.is_ok());
    }

    // ========================================
    // RED PHASE: Tests for execute_script()
    // ========================================

    #[test]
    #[cfg(target_os = "windows")]
    fn test_execute_script_simple() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();

        let result = webview.execute_script("console.log('test')");
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_execute_script_empty_fails() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();

        let result = webview.execute_script("");
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), Error::ScriptExecution(_)));
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_execute_script_returns_executed() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();

        let result = webview.execute_script("1 + 1");
        assert!(result.is_ok());
        let value = result.unwrap();
        assert!(!value.is_empty());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_execute_script_complex() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();

        let script = r#"
            (function() {
                const div = document.createElement('div');
                div.id = 'test';
                document.body.appendChild(div);
            })();
        "#;

        let result = webview.execute_script(script);
        assert!(result.is_ok());
    }

    // ========================================
    // Tests for WebView2 version
    // ========================================

    #[test]
    fn test_get_webview2_version() {
        let version = get_webview2_version();
        assert!(!version.is_empty());
        assert!(version.contains("WebView2") || version.contains("Chromium"));
    }

    #[test]
    fn test_get_webview2_version_static_method() {
        let version = WindowsWebView::get_webview2_version();
        assert!(!version.is_empty());
    }

    // ========================================
    // Tests for helper methods
    // ========================================

    #[test]
    #[cfg(target_os = "windows")]
    fn test_current_url_initially_none_or_blank() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();
        // Could be None or "about:blank" depending on mode
        let url = webview.current_url();
        assert!(url.is_none() || url == Some("about:blank".to_string()));
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_is_loading_initially_false() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();
        assert!(!webview.is_loading());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_inject_init_script() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.inject_init_script("console.log('injected');");
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_inject_init_script_empty_fails() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.inject_init_script("");
        assert!(result.is_err());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_configure_user_agent() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.configure_user_agent("CustomAgent/1.0 (Windows)");
        assert!(result.is_ok());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_configure_user_agent_empty_fails() {
        let config = WebViewConfig::default();
        let mut webview = WindowsWebView::new(config).unwrap();

        let result = webview.configure_user_agent("");
        assert!(result.is_err());
    }

    #[test]
    #[cfg(target_os = "windows")]
    fn test_setup_navigation_handlers() {
        let config = WebViewConfig::default();
        let webview = WindowsWebView::new(config).unwrap();

        let result = webview.setup_navigation_handlers();
        assert!(result.is_ok());
    }

    // ========================================
    // Integration Tests
    // ========================================

    #[test]
    #[cfg(target_os = "windows")]
    fn test_full_workflow() {
        // Create config with all options
        let temp_dir = std::env::temp_dir().join("frankenbrowser_integration_test_windows");
        let config = WebViewConfig::new()
            .with_javascript(true)
            .with_devtools(true)
            .with_user_agent("FrankenBrowser/1.0 (Windows)".to_string())
            .with_cache_dir(temp_dir.clone())
            .with_init_script("console.log('initialized');".to_string());

        // Create WebView
        let mut webview = WindowsWebView::new(config).unwrap();

        // Navigate
        let nav_result = webview.navigate("https://example.com");
        assert!(nav_result.is_ok());

        // Check URL updated
        assert_eq!(
            webview.current_url(),
            Some("https://example.com".to_string())
        );

        // Execute script
        let script_result = webview.execute_script("console.log('test')");
        assert!(script_result.is_ok());

        // Cleanup
        let _ = std::fs::remove_dir_all(&temp_dir);
    }

    // ========================================
    // Cross-platform fallback tests
    // ========================================

    #[test]
    #[cfg(not(target_os = "windows"))]
    fn test_windows_webview_on_non_windows_headless() {
        // When not on Windows, we can still test the headless implementation
        #[cfg(not(feature = "gui"))]
        {
            let config = WebViewConfig::default();
            let result = WindowsWebView::new(config);
            assert!(result.is_ok());

            if let Ok(webview) = result {
                assert!(!webview.is_loading());
                assert!(webview.current_url().is_none());
            }
        }
        #[cfg(feature = "gui")]
        {
            // In GUI mode on non-Windows, just verify the test compiles
            assert!(true);
        }
    }
}
